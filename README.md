...
Redaction reasons are explicit `reasonCodes` emitted by OPA. If a result is redacted, the `redactionReasons` array mirrors those codes and the server does not add a generic fallback reason.
